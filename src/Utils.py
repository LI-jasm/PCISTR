...
def load_our_data(dataset_str, cuda=True):
    ...
    try:
        ...
        pass
    features = torch.FloatTensor(np.array(features.todense())).float()  # 15218*15218
    labels = torch.LongTensor(labels)  # 1869
    labels = torch.max(labels, dim=1)[1]
    adj = sparse_mx_to_torch_sparse_tensor(adj).float()  # 15218*15218
...
def get_model(model_opt, nfeat, A, nhid, out, dropout=0, cuda=True):
